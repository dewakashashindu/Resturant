...
const Header = () => (
  <View style={styles.headerContainer}>
    <TouchableOpacity style={styles.hamburger} hitSlop={{ top:10,bottom:10,left:10,right:10 }}>
      <View style={styles.hamLine} /><View style={styles.hamLine} /><View style={styles.hamLine} />
    </TouchableOpacity>
...
const CalendarIcon = () => (
  <Svg width={18} height={18} viewBox="0 0 24 24">
    <Rect x={3} y={4} width={18} height={18} rx={2} stroke="#666" strokeWidth={1.5} fill="none" />
...
const DateRangeModal = ({ visible, onClose, dateRange, onConfirm }: DateRangeModalProps) => {
  ...
  React.useEffect(() => { if (visible) { setFrom(dateRange.from); setTo(dateRange.to); } }, [visible]);
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <TouchableWithoutFeedback onPress={onClose}>
        <View style={styles.modalOverlay}>
          <TouchableWithoutFeedback>
            <View style={styles.modalCard}>
              ...
              <View style={styles.dateInputRow}>
                <TextInput style={styles.dateInput} placeholder="From:" placeholderTextColor="#999" value={from} onChangeText={setFrom} />
                <View style={styles.dateIconBox}><CalendarIcon /></View>
              </View>
              <View style={styles.dateInputRow}>
                <TextInput style={styles.dateInput} placeholder="To:" placeholderTextColor="#999" value={to} onChangeText={setTo} />
                <View style={styles.dateIconBox}><CalendarIcon /></View>
              </View>
              <TouchableOpacity style={styles.confirmBtn} onPress={() => { onConfirm({ from, to }); onClose(); }}>
                <Text style={styles.confirmBtnText}>Confirm</Text>
              </TouchableOpacity>
            </View>
          </TouchableWithoutFeedback>
              ...
              {data.map(d => {
                ...
                return (
                  <View key={d.name} style={styles.svLocRow}>
                    ...
                    <View style={styles.svTrack}><View style={[styles.svFill,{width:fillW}]} /></View>
                  </View>
                );
              })}
      ...
      <View style={{ flex:1, backgroundColor:'#fff' }}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={expStyles.scrollContent}>
          <View style={expStyles.chartWrapper}>
            <PieChart donut data={pieData} radius={radius} innerRadius={innerRadius}
              innerCircleColor="#fff" showText textBackgroundRadius={0}
              isAnimated animationDuration={800} sectionAutoFocus focusOnPress strokeWidth={2} strokeColor="#fff" />
          </View>
        ...
        {stockAlerts.map((item, i) => (
          <View key={i}>
            ...
            <View style={perfStyles.stockRow}>
              ...
              <View style={[
                perfStyles.badge,
                item.level === 'Critical' ? perfStyles.badgeCritical : perfStyles.badgeLow,
              ]}>
...
const perfStyles = StyleSheet.create({
  ...
  liveOpRow: {
    flexDirection:'row',
    alignItems:'center',
    backgroundColor:'rgba(97,145,185,0.22)',
    borderRadius:10,
...
const FoodTab = () => {
  ...
  const chartDataByItem: Record<string, { day: string; value: number }[]> = {
    'Chicken Submarine': [
      ...
      { day: 'Thu', value: 80 },  { day: 'Fri', value: 70 },  { day: 'Sat', value: 110 },
      { day: 'Sun', value: 130 },
    ],
    'Mango Juice': [
      { day: 'Mon', value: 90 },  { day: 'Tue', value: 140 }, { day: 'Wed', value: 100 },
      { day: 'Thu', value: 60 },  { day: 'Fri', value: 130 }, { day: 'Sat', value: 150 },
      { day: 'Sun', value: 95 },
    ],
    'Water Bottle': [
      { day: 'Mon', value: 60 },  { day: 'Tue', value: 80 },  { day: 'Wed', value: 70 },
      { day: 'Thu', value: 90 },  { day: 'Fri', value: 100 }, { day: 'Sat', value: 120 },
      { day: 'Sun', value: 85 },
    ],
    'Chicken Pasta': [
      { day: 'Mon', value: 100 }, { day: 'Tue', value: 130 }, { day: 'Wed', value: 160 },
      { day: 'Thu', value: 110 }, { day: 'Fri', value: 95 },  { day: 'Sat', value: 140 },
      { day: 'Sun', value: 105 },
    ],
    'Fish Noodles': [
      { day: 'Mon', value: 70 },  { day: 'Tue', value: 95 },  { day: 'Wed', value: 120 },
      { day: 'Thu', value: 65 },  { day: 'Fri', value: 88 },  { day: 'Sat', value: 100 },
      { day: 'Sun', value: 75 },
    ],
  };
  ...
  const barData = chartData.map(d => ({
    ...
    frontColor: '#AAC3D9',
    topLabelComponent: () => (
      <Text style={{ fontSize: isTablet ? 12 : 10, color: '#333', marginBottom: 4 }}>
        {d.value}
      </Text>
    ),
  }));
          ...
          {items.map(item => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  foodStyles.chip,
                  { backgroundColor: isSelected ? 'rgba(0,98,170,0.56)' : 'rgba(97,145,185,0.54)' },
                ]}
                activeOpacity={0.7}
              >
          ...
          <View style={{ width: 32, marginRight: 4, alignItems: 'flex-end' }}>
            <View style={{ height: chartHeight, justifyContent: 'space-between' }}>
              {yAxisLabels.map((val, i) => (
                <Text
                  key={i}
                  style={{ fontSize: yLabelFs, color: '#54555A', lineHeight: yLabelFs + 2 }}
                >
                  {val}
                </Text>
              ))}
...
const FoodItemExpanded = ({
  ...
  const barData = chartData.map(d => ({
    ...
    frontColor: '#AAC3D9',
    topLabelComponent: () => (
      <Text style={{ fontSize: isTablet ? 13 : 11, color: '#333', marginBottom: 4 }}>
        {d.value}
      </Text>
    ),
  }));
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginTop: 24 }}>
            <View style={{ width: 36, marginRight: 4, alignItems: 'flex-end' }}>
              <View style={{ height: chartHeight, justifyContent: 'space-between' }}>
                {yAxisLabels.map((val, i) => (
                  <Text key={i} style={{ fontSize: yLabelFs, color: '#54555A', lineHeight: yLabelFs + 2 }}>
                    {val}
                  </Text>
                ))}
              </View>
    ...
    <View style={styles.kpiOuterCard}>
      <View style={styles.kpiGrid}>
        {visibleCards.map((c,i) => {
          ...
          return (
            <W key={i} style={[styles.kpiInnerCard,{width:innerCardW,backgroundColor:c.bg}]}
      ...
      <View style={styles.kpiMoreRow}>
        <TouchableOpacity style={styles.moreBtn} onPress={() => setShowMore(p => !p)}>
          ...
          <Svg width={14} height={14} viewBox="0 0 24 24">
            <Path d={showMore ? 'M6 15l6-6 6 6' : 'M6 9l6 6 6-6'} stroke="#333" strokeWidth={2} fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </Svg>
        </TouchableOpacity>
      </View>
    </View>
...
const SalesVolumeChart = () => {
  const barWidth=isTablet?18:14, spacing=isTablet?20:16, barSpacing=3;
  ...
  const yLabelFs=isTablet?11:9, xLabelAreaH=24;
  return (
    <View style={styles.sectionCard}>
      ...
      <View style={{ flexDirection:'row', alignItems:'flex-start' }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} bounces={false} style={{ flex:1 }}>
          <BarChart data={barData} maxValue={maxValue} noOfSections={noOfSections}
            barBorderRadius={4} yAxisThickness={0} yAxisTextStyle={{ color:'transparent', fontSize:1 }}
            yAxisLabelWidth={0} xAxisThickness={1} xAxisColor="#E0E0E0"
            xAxisLabelTextStyle={{ color:'#54555A', fontSize: isTablet?11:9, width:44, textAlign:'center' }}
            isAnimated animationDuration={800} rulesType="solid" rulesColor="#F0F0F0"
            height={chartHeight} width={chartWidth} initialSpacing={12} endSpacing={12}
            showGradient={false} disableScroll />
        </ScrollView>
      </View>
    </View>
  );
...
const SalesDistributionCard = () => {
  const [expandedVisible,setExpandedVisible]=useState(false);
  const radius=isTablet?95:isSmall?65:80, innerRadius=isTablet?58:isSmall?38:48;
  const pieData=[
  ...
  const legend=[
    {color:'#2E2855',label:'Dining'},{color:'#8AB4FF',label:'Take Away'},
    {color:'#703DDE',label:'Pick Up'},{color:'#3F96D4',label:'Delivery'},
  ];
  return (
    <>
      <View style={styles.sectionCard}>
...
const TopPerformanceCard = () => {
  ...
  const data=[
    {name:'Crimson\nChopsticks',value:709},{name:'Basil & Barrel',value:620},
    {name:'The Olive Grove',value:583},{name:'The Golden Wok',value:425},
  ];
  return (
    <>
      <View style={styles.sectionCard}>
...
const PaymentTypeCard = () => {
  const radius=isTablet?110:90, innerRadius=isTablet?65:54;
  const pieData=[
    {value:40,color:'#006BD6'},{value:30,color:'#537FF1'},{value:15,color:'#FF6F5A'},
    {value:10,color:'#3CC3DF'},{value:3,color:'#FF1301'},{value:2,color:'#5A4DC0'},
  ];
  const legend: {color:string;label:string}[]=[
    {color:'#5A4DC0',label:'GIFT VOUCHER'},{color:'#FF1301',label:'CREDIT'},
    {color:'#3CC3DF',label:'CASH'},{color:'#FF6F5A',label:'AMEX CARD'},
    {color:'#537FF1',label:'MASTER CARD'},{color:'#006BD6',label:'VISA CARD'},
  ];
  return (
    <View style={styles.sectionCard}>
      <View style={styles.cardHeaderRow}>
      ...
      <View style={{ alignItems:'center', marginVertical:8 }}>
        <PieChart donut data={pieData} radius={radius} innerRadius={innerRadius}
          innerCircleColor="#fff" isAnimated animationDuration={800}
          sectionAutoFocus focusOnPress strokeWidth={2} strokeColor="#fff"
...
const MonthlySalesTrend = () => {
  const chartH=isTablet?220:isSmall?130:170, pointW=isTablet?60:50;
  ...
  return (
    <View style={[styles.sectionCard,{paddingBottom:12}]}>
      ...
      <View style={{ flexDirection:'row', alignItems:'flex-start' }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} bounces={false} style={{ flex:1 }}>
          <LineChart data={lineData} width={chartWidth} height={chartH}
            maxValue={maxValue} noOfSections={noOfSections}
            areaChart curved color="#5D51A9" thickness={2.5}
            startFillColor="rgba(137,121,255,0.5)" endFillColor="rgba(137,121,255,0.02)"
            startOpacity={0.8} endOpacity={0.1}
            initialSpacing={16} endSpacing={16} spacing={pointW}
            dataPointsColor="#5D51A9" dataPointsRadius={4}
            yAxisColor="transparent" xAxisColor="#E0E0E0"
            yAxisThickness={0} yAxisTextStyle={{ color:'transparent', fontSize:1 }}
            yAxisLabelWidth={0}
            xAxisLabelTextStyle={{ color:'#54555A', fontSize:isTablet?11:9 }}
            rulesType="solid" rulesColor="#F0F0F0" isAnimated animationDuration={1000}
            focusEnabled showStripOnFocus stripColor="rgba(93,81,169,0.2)"
            stripWidth={2} unFocusOnPressOut disableScroll />
        </ScrollView>
      </View>
    </View>
  );
...
// ─── Horizontal Bar Chart for Reviews ────────────────────────────────────────
const ReviewBarChart = ({ data }: {
  data: { star: number; count: number }[];
}) => {
  const MAX_COUNT  = 200;
  const TRACK_W    = CARD_WIDTH - 32 - 56 - 36 - 12; // card padding + left labels + right labels + gaps
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <View>
          ...
          <Text style={custStyles.reviewMeta}>
            {timeAgo} · {restaurant}
          </Text>
        </View>
      </View>
  ...
  const reviews = [
    {
      ...
      text:       'Service was fast and the chicken biryani tasted amazing. Will definitely order again.',
    },
    {
      name:       'Sanduni Perera',
      timeAgo:    '2 days ago',
        ...
        <View style={{ padding: 16, gap: 0 }}>
          {restaurants.map((r, i) => (
            <RestaurantReviewCard
              ...
              barData={[...r.barData].reverse()} // render 5→1 top to bottom
...
const custStyles = StyleSheet.create({
  ...
  reviewsCardHeader: {
    backgroundColor:  'rgba(97,145,185,0.34)',
    paddingHorizontal: 16,
    paddingVertical:  14,
  },
  reviewsCardHeaderText: {
  ...
  return (
    <View style={styles.tabBarWrapper}>
      {tabs.map(tab=>{
        ...
        return (
          <TouchableOpacity key={tab} style={[styles.tabItem,isActive&&styles.activeTab]}
            onPress={()=>onTabChange(tab)} activeOpacity={0.8}>
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.topArea}>
        ...
        <FilterRow
          dateLabel={dateLabel} locationLabel={locationLabel}
      ...
      <LocationModal visible={locationModalVisible} onClose={()=>setLocationModalVisible(false)}
        selected={selectedLocations} onConfirm={sel=>setSelectedLocations(sel)} />
      <DateRangeModal visible={dateModalVisible} onClose={()=>setDateModalVisible(false)}
        dateRange={dateRange} onConfirm={range=>setDateRange(range)} />
      <SalesVolumeModal visible={salesVolumeModalVisible} onClose={()=>setSalesVolumeModalVisible(false)} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  dateInputRow:    { flexDirection:'row', alignItems:'center', gap:10, marginBottom:14 },
  dateInput:       { flex:1, borderWidth:1, borderColor:'#CCC', borderRadius:24, paddingHorizontal:16, paddingVertical:12, fontSize:isTablet?15:14, color:'#000' },
  dateIconBox:     { width:34, height:34, justifyContent:'center', alignItems:'center', borderWidth:1, borderColor:'#CCC', borderRadius:6 },
